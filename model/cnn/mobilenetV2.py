...
def conv_bn(inp, oup, stride):
    return nn.Sequential(
        nn.Conv1d(inp, oup, 3, stride, 1, bias=True),
        nn.BatchNorm1d(oup),
...
class InvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        ...
        self.conv = nn.Sequential(
            ...
            nn.Conv1d(inp, hidden_dim, 1, 1, 0, bias=True),
            nn.BatchNorm1d(hidden_dim),
            nn.ReLU6(inplace=True),
            # depth-wise convolution
            nn.Conv1d(hidden_dim, hidden_dim, 3, stride, 1, groups=hidden_dim, bias=True),
            nn.BatchNorm1d(hidden_dim),
            nn.ReLU6(inplace=True),
            # point-wise linear convolution
            nn.Conv1d(hidden_dim, oup, 1, 1, 0, bias=True),
            nn.BatchNorm1d(oup),
        )
